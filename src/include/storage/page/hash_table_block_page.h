...
class HashTableBlockPage {
 ...
 private:
  void SetOccupied(slot_offset_t index, bool val);
  void SetReadable(slot_offset_t index, bool val);
  bool GetBit(std::atomic_char *array,slot_offset_t index) const;
  void SetBit(std::atomic_char *array,slot_offset_t index,bool val);
  mutable std::atomic_char occupied_[(BLOCK_ARRAY_SIZE - 1) / 8 + 1];

  // 0 if tombstone/brand new (never occupied), 1 otherwise.
  mutable std::atomic_char readable_[(BLOCK_ARRAY_SIZE - 1) / 8 + 1];
